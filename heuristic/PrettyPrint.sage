...
def pretty_print_ring(field_desc, index):
    ...
    if len(field_desc) == 2:
        ...
        if index == 1:
            if discriminant % 4 == 0:
                return 'ZZ [sqrt(%s)]'% str(discriminant//4)
            return 'ZZ [(1 + sqrt(%s))/2]' % str(discriminant)
        if discriminant % 4 == 0:
            return 'ZZ [%s sqrt(%s)]' % (str(index), str(discriminant//4))
        if index % 2 == 0:
            return 'ZZ [%s sqrt(%s)]' % (str(index//2), str(discriminant))
        return 'ZZ [(1 + sqrt(%s))/2]' % (str(index), str(discriminant))
    else:
        if index == 1:
...
def statements_all(desc_dict, genus, fieldstring):
    statements= [ ]
    statements.append(statement_endomorphisms_QQ(desc_dict, genus, fieldstring) + ' (%s)'.format(statement_cm(desc_dict, genus, fieldstring)))
    statements.append(statement_endomorphisms_ZZ(desc_dict, genus, fieldstring) + ' (%s)'.format(statement_eichler(desc_dict, genus, fieldstring)))
...
def statement_endomorphisms_RR(desc_dict, genus, fieldstring):
    return """End (J_%s) ox RR: %s\n""" % (fieldstring, ' x '.join(desc_dict['desc_RR']))
...
def statement_factor_QQ(factor_QQ):
    ...
    if factor_QQ['albert_type'] == 'I':
        ...
        disc = factor_QQ['discriminant']
        if disc == 1:
            statement = "M_%s (%s)""" % (d, d, base_str)
        elif d == 2:
            statement = "IndefQuat (%s, %s)"  % (base_str, disc)
        else:
            statement = "IndefAlg_%s (%s, %s)"  % (d, base_str, disc)

    elif factor_QQ['albert_type'] == 'III':
        d = factor_QQ['dim_sqrt']
        disc = factor_QQ['discriminant']
        if d == 2:
            statement = "DefQuat (%s, %s)"  % (base_str, disc)
        else:
            statement = "DefAlg_%s (%s, %s)"  % (d, base_str, disc)
        ...
        if disc == 1:
            ...
            statement = "Quat (%s, %s)"  % (base_str, disc)
        else:
            statement = "Alg_%s (%s, %s)"  % (d, base_str, disc)
...
def statement_factor_ZZ_maximal(factor_QQ, desc_ZZ, fieldstring):
    ...
    elif factor_QQ['albert_type'] == 'II' and factor_QQ['discriminant'] == index and pretty_print_field(factor_QQ['base_field']) == 'QQ':
        return 'M_%s (%s)' % (factor_QQ['dim_sqrt'], pretty_print_ring(factor_QQ['base_field'], index))
    elif factor_QQ['albert_type'] == 'IV' and factor_QQ['discriminant'] == index and pretty_print_field(factor_QQ['base_field']) == 'QQ':
        return 'M_%s (%s)' % (factor_QQ['dim_sqrt'], pretty_print_ring(factor_QQ['base_field'], index))
    else:
        return 'Max (%s)' % statement_factor_QQ(factor_QQ)

def statement_factors_ZZ_index(factors_QQ, desc_ZZ):
    if len(desc_dict['factors_QQ']) == 1:
        factor_QQ = desc_dict['factors_QQ'][0]
        statement = "%s" % pretty_print_ring(factor_QQ['base_field'], desc_ZZ['index'])
    else:
...
def statement_eichler(desc_dict, genus, fieldstring):
    factors_QQ = desc_dict['factors_QQ']
    if len(factors_QQ) == 1 and factors_QQ['albert_type'] != 'I':
        if desc_dict['desc_ZZ']['is_eichler'] == 1:
            return "Eichler"
    return ""
...
def statement_simple(desc_dict, genus, fieldstring):
    ...
    if not len(factors_QQ) == 1:
        ...
        if factor_QQ['dim_sqrt'] > 1 and factor_QQ['discriminant'] != 1:
            return "simple"
    return "not simple"
